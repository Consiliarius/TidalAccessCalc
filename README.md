...
- **Date** (DD/MM/YYYY)
- **Time (BST)** — local time as shown on your watch or phone
- **Wind Direction** — from the dropdown
- **Afloat?** — Yes, No, or Uncertain
- **Notes** — any relevant detail (e.g. "just touching", "well afloat", "heeled on mud")

The most valuable observations are those taken near low water, particularly on spring tides (constrains the maximum drying height) and neap tides (constrains the minimum). Transitional moments — when the boat is just lifting off or settling — provide the tightest constraints.

### Uploading observations

1. Export the Observations sheet from the spreadsheet as CSV.
2. In the dashboard, go to the **Observations** tab.
3. Upload or paste the CSV.

Observation times are recorded in BST (local time). The dashboard converts them to UTC automatically on import to align with the tidal data.
